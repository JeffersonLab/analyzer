...
Int_t UserScintillator::ReadDatabase( const TDatime& date )
{
  ...
  fC = fCn;           // default set in parent class
  fTDCscale = fTdc2T; // default set in parent class
  Int_t stop = 1;
  ...
  try {
    ...
    // All parameters are optional since we can set more or less sensible defaults
    const DBRequest request[] = {
      { "c",        &fC,        kDouble, 0, 1 }, // Speed of light in medium
      { "tdcscale", &fTDCscale, kDouble, 0, 1 }, // TDC scale (ns/chan)
      { "stop",     &stop,      kInt,    0, 1 }, // Common stop mode (1=yes)
...
Int_t UserScintillator::FineProcess( TClonesArray& tracks )
{
  ...
  // Compute y-position from TDC data if possible
    ...
    if( fCommonStop )
      fYtdc = 0.5*fC*(fRT_c[fPaddle]-fLT_c[fPaddle]);
    else
      fYtdc = 0.5*fC*(fLT_c[fPaddle]-fRT_c[fPaddle]);
  }
