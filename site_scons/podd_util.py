...
def build_library(env, sotarget, src, extrahdrs = [], extradicthdrs = [],
                  ...
                  install_rpath = [], destdir = '.'):
    '''
    Build shared library lib<sotarget> of ROOT classes from given sources "src"
    (space separated string). For each .C or .cxx source file, a .h header file is
    expected.
    ...
    If the dictionary source file should have a different name than <sotarget>Dict.C,
    ...
    rootdict = dictname+'Dict.C'
    libbase = env.subst('$SHLIBPREFIX')+sotarget
    thedict = env.RootCint(rootdict, dicthdrs,
                           PCMNAME=os.path.join(destdir,libbase))
    ...
    # Build the library
    thislib = env.SharedLibrary(target = os.path.join(destdir,sotarget),
    ...
    if versioned:
        # Create symlinks
        env.SymLink(os.path.join(destdir,libname_soname),thislib)
        env.SymLink(os.path.join(destdir,libname_so),
                    os.path.join(destdir,libname_soname))
