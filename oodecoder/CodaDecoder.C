...
//_____________________________________________________________________________
Int_t CodaDecoder::LoadEvent(const Int_t* evbuffer)
{//PUBLIC
 ...
 if (fDebugFile) *fDebugFile << "CodaDecode:: Loading event  ... "<<endl;
  ...
  if (first_decode || fgNeedInit) {
    ret = init_cmap();
    if (fDebugFile) {
         *fDebugFile << "CodaDecode:: Print of Crate Map"<<endl;
...
//_____________________________________________________________________________
Int_t CodaDecoder::roc_decode( Int_t roc, const Int_t* evbuffer,
...
//_____________________________________________________________________________
Int_t CodaDecoder::LoadIfFlagData(const Int_t* evbuffer)
{
...
Int_t CodaDecoder::FindRocs(const Int_t *evbuffer) {
...
int CodaDecoder::init_slotdata(const THaCrateMap* map)
{
  ...
  for( int i=0; i<fNSlotUsed; i++ ) {
    ...
    if (fDebugFile) *fDebugFile << "CodaDecode:: Dev type  = "<<crslot->devType()<<endl;
...
//_____________________________________________________________________________
void CodaDecoder::dump(const Int_t* evbuffer)
{
  if( !evbuffer ) return;
...
//_____________________________________________________________________________
void CodaDecoder::ChkFbSlot( Int_t roc, const Int_t* evbuffer,
				  Int_t ipt, Int_t istop )
{
  const Int_t* p      = evbuffer+ipt;    // Points to ROC ID word (1 before data)
