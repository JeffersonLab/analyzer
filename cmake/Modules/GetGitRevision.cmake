...
function(_git_date_to_shortdate _date _shortdate)
    set(_fields "${_date}")
    separate_arguments(_fields)
    list(LENGTH _fields len)
    if(len GREATER_EQUAL 4)
        list(GET _fields 1 _day)
        list(GET _fields 2 _month)
        list(GET _fields 3 _year)
        set(${_shortdate} "${_day} ${_month} ${_year}" PARENT_SCOPE)
    else()
        set(${_shortdate} "${_date}" PARENT_SCOPE)
    endif()
endfunction()
...
function(get_git_info _hashvar _shortdatevar _datevar)
    ...
    if(res EQUAL 0)
        _git_date_to_shortdate("${out}" _dateout)
        set(${_shortdatevar} "${_dateout}" PARENT_SCOPE)
    else()
