...
private:
  ...
  ULong64_t   evnum;     // Most recent physics event number seen
...
// Get tagged value nearest 'event'
   Double_t GetData( const char* tag, ULong64_t event= 0 ) const;
// Get tagged string value nearest 'event'
   std::string GetString( const char* tag, ULong64_t event= 0 ) const;
   time_t GetTimeStamp( const char* tag, ULong64_t event= 0 ) const;
   Int_t LoadData( const UInt_t* evbuffer, ULong64_t event= 0 );  // load the data
...
private:
  ...
  static UInt_t FindEvent( const std::vector<EpicsChan>& ep, ULong64_t event );
