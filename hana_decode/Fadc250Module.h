...
public:
  ...
  virtual UInt_t GetData(Decoder::EModuleType mtype, UInt_t chan,
                         UInt_t ievent) const;
  ...
  virtual UInt_t GetNumEvents(UInt_t ichan) const {
    return GetNumFadcEvents(ichan);
  };
  virtual UInt_t GetNumSamples(UInt_t ichan) const {
    return GetNumFadcSamples(ichan, 0);
  };
...
private:
  ...
  struct fadc_data_struct {
    // Header data objects
    uint32_t slot_blk_hdr, mod_id, iblock_num,
        nblock_events;                     // Block header objects
    ...
    bool overflow, invalid_samples; // True if any sample's "overflow" or "not
                                    // valid" bit set, respectively
    // Pulse raw data objects
    uint32_t pulse_num,
        sample_num_tc; // FADC pulse number, sample number of threshold crossing
    // Pulse integral data objects
    uint32_t qual_factor, pulse_integral; // FADC quality factor, pulse integral
    // Pulse time data objects
    uint32_t coarse_pulse_time, fine_pulse_time,
        time; // FADC pulse coarse time, pulse fine time, pulse time
    // Pulse pedestal data objects
    uint32_t pedestal, pulse_peak,
        pedestal_sum; // FADC pedestal, pulse peak value
    // Scaler data objects
    ...
    uint32_t samp_overflow, samp_underflow,
        samp_over_thresh; // FADC pulse parameters
    // Time word object
    uint32_t peak_beyond_nsa, peak_not_found,
        peak_above_maxped; // FADC pulse parameters
  ...
  struct fadc_scaler_data {
    uint32_t
        scaler_words;     // Total number of scaler words present (should be 18)
    uint32_t scaler_time; // Timestamp of scaler values
    uint32_t scaler_trigger_count; // Trigger count which scalers were read

    std::vector<uint32_t> scaler_values; // Size should be scaler_words - 2 = 16
    void clear() {
      scaler_words = 0;
      scaler_time = 0;
      scaler_trigger_count = 0;
      scaler_values.clear();
    }
  } __attribute__((aligned(128)));
  std::vector<fadc_scaler_data> fScalerData;

  // State tracking variable and data vector for scaler data
  UInt_t fScalersToRead;

  Bool_t data_type_4, data_type_6, data_type_7, data_type_8, data_type_9,
      data_type_10;
  Bool_t block_header_found, block_trailer_found, event_header_found,
      slots_match;

  void ClearDataVectors();
  void PopulateDataVector(std::vector<uint32_t> &data_vector,
                          uint32_t data) const;
