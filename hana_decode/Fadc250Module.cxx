...
//
//   The following data type summary corresponds to firmware version 0x0C00
//   (06/09/2016)
//   -----------------------------------------------------------------------------------
//   0 block header
//   1 block trailer
...
Bool_t Fadc250Module::HasCapability(Decoder::EModuleType type) {
  return (type == kSampleADC || type == kPulseIntegral || type == kPulseTime ||
          type == kPulsePeak || type == kPulsePedestal || type == kCoarseTime ||
          type == kFineTime);
}

//_____________________________________________________________________________
// Clear all data vectors
inline void Fadc250Module::ClearDataVectors() {
...
inline void Fadc250Module::PopulateDataVector(vector<uint32_t> &data_vector,
                                              uint32_t data) const {
  if (slots_match)
...
inline uint32_t
Fadc250Module::SumVectorElements(const vector<uint32_t> &data_vector) {
...
void Fadc250Module::Clear(Option_t *opt) {
  ...
  data_type_4 = data_type_6 = data_type_7 = data_type_8 = data_type_9 =
      data_type_10 = false;
  block_header_found = block_trailer_found = event_header_found = slots_match =
      false;
  // Scaler data initialization
  fScalerData.clear();
  fScalersToRead = 0;
}
...
UInt_t Fadc250Module::GetPulseIntegralData(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetPulseIntegralData:: invalid event "
            "number for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulseIntegralData:: data vector empty "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseIntegralData channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetEmulatedPulseIntegralData(UInt_t chan) const {
  ...
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetEmulatedPulseIntegralData:: data "
            "vector empty  for slot = "
         << fSlot << ", channel = " << chan << "\n"
    ...
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetEmulatedPulseIntegralData channel "
                  << chan << " = "
                  << SumVectorElements(fPulseData[chan].samples) << endl;
#endif
...
UInt_t Fadc250Module::GetPulseTimeData(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetPulseTimeData:: invalid event number "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulseTimeData:: data vector empty for "
            "slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseTimeData channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetPulseCoarseTimeData(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetPulseCoarseTimeData:: invalid event "
            "number for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulseCoarseTimeData:: data vector "
            "empty for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseCoarseTimeData channel " << chan
                  << ", event " << ievent << " = "
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseFineTimeData channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetPulsePeakData(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetPulsePeakData:: invalid event number "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulsePeakData:: data vector empty for "
            "slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulsePeakData channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetPulsePedestalData(UInt_t chan, UInt_t ievent) const {
  ...
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulsePedestalData:: data vector empty "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (fFirmwareVers >= 2) {
    if (ievent >= nevent) {
      cout << "ERROR:: Fadc250Module:: GetPulsePedestalData:: invalid data "
              "vector size = "
           << fSlot << ", channel = " << chan << endl;
      return kMaxUInt;
    } else {
#ifdef WITH_DEBUG
      if (fDebugFile)
        *fDebugFile << "Fadc250Module::GetPulsePedestalData channel " << chan
                    << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetPedestalQuality(UInt_t chan, UInt_t ievent) const {
  ...
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPedestalQuality:: data vector empty "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (fFirmwareVers >= 2) {
    if (ievent >= nevent) {
      cout << "ERROR:: Fadc250Module:: GetPulsePedestalQuality:: invalid data "
              "vector size = "
           << fSlot << ", channel = " << chan << endl;
      return kMaxUInt;
    } else {
#ifdef WITH_DEBUG
      if (fDebugFile)
        *fDebugFile << "Fadc250Module::GetPulsePedestalQuality channel " << chan
                    << ", event " << ievent << " = "
  ...
  if (nevent != 1) {
    cout << "ERROR:: Fadc250Module:: GetPedestalQuality:: invalid data vector "
            "size = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPedestalQuality channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetOverflowBit(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetOverflowBit:: invalid event number for "
            "slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetOverflowBit:: data vector empty for "
            "slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetOverflowBit channel " << chan
                  << ", event " << ievent << " = "
...
UInt_t Fadc250Module::GetUnderflowBit(UInt_t chan, UInt_t ievent) const {
  ...
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetUnderflowBit:: invalid event number "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetUnderflowBit:: data vector empty for "
            "slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetUnderflowBit channel " << chan
                  << ", event " << ievent << " = "
...
#ifdef WITH_DEBUG
  if (fDebugFile)
    *fDebugFile << "Fadc250Module::GetTriggerTime = " << fadc_data.trig_time
                << " " << shorttime << endl;
#endif
  return shorttime;
}
...
UInt_t Fadc250Module::GetPulseSamplesData(UInt_t chan, UInt_t ievent) const {
  vsiz_t nevent = fPulseData[chan].samples.size();
  if (ievent >= nevent) {
    cout << "ERROR:: Fadc250Module:: GetPulseSamplesData:: invalid event "
            "number for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulseSamplesData:: data vector empty "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseSamplesData channel " << chan
                  << ", event " << ievent << " = "
...
vector<uint32_t> Fadc250Module::GetPulseSamplesVector(UInt_t chan) const {
  vsiz_t nevent = fPulseData[chan].samples.size();
  if (nevent == 0) {
    cout << "ERROR:: Fadc250Module:: GetPulseSamplesVector:: data vector empty "
            "for slot = "
         << fSlot << ", channel = " << chan << endl;
    return {};
  } else {
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetPulseSamplesVector channel " << chan
                  << " = " << &fPulseData[chan].samples << endl;
#endif
...
void Fadc250Module::PrintDataType() const {
  ...
  *fDebugFile << "start,  Print Data Type " << endl;
  if (data_type_4)
    *fDebugFile << "data type 4" << endl;
  if (data_type_6)
    *fDebugFile << "data type 6" << endl;
  if (data_type_7)
    *fDebugFile << "data type 7" << endl;
  if (data_type_8)
    *fDebugFile << "data type 8" << endl;
  if (data_type_9)
    *fDebugFile << "data type 9" << endl;
  if (data_type_10)
    *fDebugFile << "data type 10" << endl;
...
Int_t Fadc250Module::GetFadcMode() const {
  if (fFirmwareVers == 1) { // some "older" firmware
    if (data_type_4 && data_type_6)
      return 8;
    if (data_type_7)
      return 7;
  }
  if (data_type_4 && !(data_type_6) && !(data_type_7) && !(data_type_8) &&
      !(data_type_9) && !(data_type_10))
    return 1;
  else if (!(data_type_4) && data_type_6 && !(data_type_7) && !(data_type_8) &&
           !(data_type_9) && !(data_type_10))
    return 2;
  else if (!(data_type_4) && !(data_type_6) && data_type_7 && data_type_8 &&
           !(data_type_9) && !(data_type_10))
    ...
    return 7;
           ...
           !(data_type_9) && data_type_10)
    return 8;
  else if (!(data_type_4) && !(data_type_6) && !(data_type_7) &&
           !(data_type_8) && data_type_9 && !(data_type_10))
    return 9;
  else if (data_type_4 && !(data_type_6) && !(data_type_7) && !(data_type_8) &&
           data_type_9 && !(data_type_10))
    return 10;
    ...
    //      PrintDataType();
    cout << "ERROR:: Fadc250Module:: GetFadcMode:: FADC is in invalid mode for "
            "slot = "
         << fSlot << endl;
    if (fDebugFile)
      *fDebugFile << "ERROR:: Fadc250Module:: GetFadcMode:: FADC is in invalid "
                     "mode for slot = "
                  << fSlot << endl;
    return -1;
  }
...
UInt_t Fadc250Module::GetNumFadcEvents(UInt_t chan) const {
  ...
  if (mode == 1)
    return 1;
            ...
            ((sz = fPulseData[chan].integral.size()) ==
             fPulseData[chan].time.size()) &&
            (fPulseData[chan].pedestal.size() == sz) &&
            (fPulseData[chan].peak.size() == sz)) ||
           (mode == 8 &&
            (sz = fPulseData[chan].time.size()) ==
                fPulseData[chan].pedestal.size() &&
            fPulseData[chan].peak.size() == sz) ||
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::GetNumFadcEvents channel " << chan << " = "
                  << sz << endl;
#endif
    return sz;
  } else {
    cout << "ERROR:: Fadc250Module:: GetNumFadcEvents:: FADC not in acceptable "
            "mode or data vector sizes do not match for for slot = "
         << fSlot << ", channel = " << chan << endl;
    cout << "ERROR:: Fadc250Module:: GetNumFadcEvents:: FADC data potentially "
            "corrupt for event "
         << fadc_data.evt_num << ", PROCEED WITH CAUTION!" << endl;
    return kMaxUInt;
  }
}
...
UInt_t Fadc250Module::GetNumFadcSamples(UInt_t chan, UInt_t ievent) const {
  ...
  if ((mode == 1) || (mode == 8) || (mode == 10)) {
    ...
    if (ievent >= nsamples) {
      cout << "ERROR:: Fadc250Module:: GetNumFadcSamples:: invalid event "
              "number for slot = "
           << fSlot << ", channel = " << chan << endl;
      return kMaxUInt;
    }
    if (nsamples == 0) {
      // cout << "ERROR:: Fadc250Module:: GetNumFadcSamples:: data vector empty
      // for slot = " << fSlot << ", channel = " << chan << endl;
      return kMaxUInt;
    } else {
#ifdef WITH_DEBUG
      if (fDebugFile)
        *fDebugFile << "Fadc250Module::GetNumFadcSamples channel " << chan
                    << ", event " << ievent << " = "
    ...
    cout << "ERROR:: Fadc250Module:: GetNumFadcSamples:: FADC is not in Mode "
            "1, 8, or 10 for slot = "
         << fSlot << ", channel = " << chan << endl;
    return kMaxUInt;
  }
...
//_____________________________________________________________________________
inline void Fadc250Module::DecodeBlockHeader(UInt_t pdat,
                                             uint32_t data_type_id) {
  static const string here{"Fadc250Module::DecodeBlockHeader"};

  if (data_type_id) {
    block_header_found = true; // Set to true if found
    fadc_data.slot_blk_hdr =
        (pdat >> 22) & 0x1F; // Slot number (set by VME64x backplane), mask 5
                             // bits Debug output
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::Decode:: Slot from FADC block header = "
                  << fadc_data.slot_blk_hdr << endl;
#endif
    ...
    if (!slots_match)
      return;
    fadc_data.mod_id =
        (pdat >> 18) & 0xF; // Module ID ('1' for FADC250), mask 4 bits
    fadc_data.iblock_num =
        (pdat >> 8) & 0x3FF; // Event block number, mask 10 bits
    fadc_data.nblock_events = (pdat >> 0) & 0xFF; // Number of events in block,
                                                  // mask 8 bits Debug output
#ifdef WITH_DEBUG
    if (fDebugFile)
    ...
    fadc_data.PL =
        (pdat >> 18) & 0x7FF; // Number of samples before trigger point for
                              // processing to begin, mask 11 bits
    fadc_data.NSB =
        (pdat >> 9) & 0x1FF; // Number of samples before threshold crossing to
                             // include processing, mask 9 bits
    fadc_data.NSA = (pdat >> 0) & 0x1FF; // Number of samples after threshold
                                         // crossing to include processing, mask
                                         // 9 bits Debug output
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::Decode:: FADC BLOCK HEADER"
...
void Fadc250Module::DecodeBlockTrailer(UInt_t pdat) {
  ...
  fadc_data.slot_blk_trl =
      (pdat >> 22) & 0x1F; // Slot number (set by VME64x backplane), mask 5 bits
  fadc_data.nwords_inblock =
      (pdat >> 0) &
      0x3FFFFF; // Total number of words in block of events, mask 22 bits
  // Debug output
#ifdef WITH_DEBUG
  if (fDebugFile)
...
void Fadc250Module::DecodeEventHeader(UInt_t pdat) {
  ...
  fadc_data.slot_evt_hdr =
      (pdat >> 22) & 0x1F; // Slot number (set by VME64x backplane), mask 5 bits
  // fadc_data.evt_num = (pdat >> 0) & 0x3FFFFF;    // Event number, mask 22
  // bits For firmware versions post 0x0C00 (06/09/2016)
  fadc_data.eh_trig_time = (pdat >> 12) & 0x3FF; // Event header trigger time
  fadc_data.trig_num = (pdat >> 0) & 0xFFF;      // Trigger number
  // Debug output
  // #ifdef WITH_DEBUG
  //	if (fDebugFile)
  //	  *fDebugFile << "Fadc250Module::Decode:: FADC EVENT HEADER >> data = "
  //<< hex
...
void Fadc250Module::DecodeWindowRawData(UInt_t pdat, uint32_t data_type_id) {
  ...
  if (data_type_id == 1) {
    ...
    if (!invalid_1)
      sample_1 = (pdat >> 16) & 0x1FFF; // If sample x is valid, assign value
    if (!invalid_2)
      sample_2 = (pdat >> 0) & 0x1FFF; // If sample x+1 is valid, assign value

    PopulateDataVector(fPulseData[fadc_data.chan].samples,
                       sample_1);                // Sample 1
...
void Fadc250Module::DecodePulseRawData(UInt_t pdat, uint32_t data_type_id) {
  ...
  if (data_type_id == 1) {
    ...
    fadc_data.sample_num_tc =
        (pdat >> 0) & 0x3FF; // FADC sample number of threshold crossing
                             // Debug output
#ifdef WITH_DEBUG
    if (fDebugFile)
...
void Fadc250Module::DecodePulseIntegral(UInt_t pdat) {
  ...
  PopulateDataVector(fPulseData[fadc_data.chan].integral,
                     fadc_data.pulse_integral);
  // Debug output
#ifdef WITH_DEBUG
  if (fDebugFile)
...
void Fadc250Module::DecodePulseTime(UInt_t pdat) {
  ...
  fadc_data.pulse_num = (pdat >> 21) & 0x3;   // FADC pulse number
  fadc_data.qual_factor = (pdat >> 19) & 0x3; // FADC quality factor (0-3)
  fadc_data.coarse_pulse_time =
      (pdat >> 6) & 0x1FF; // FADC coarse time (4 ns/count)
  fadc_data.fine_pulse_time =
      (pdat >> 0) & 0x3F;                // FADC fine time (0.0625 ns/count)
  fadc_data.time = (pdat >> 0) & 0x7FFF; // FADC time (0.0625 ns/count, bmoffit)
  // Store data in arrays of vectors
  PopulateDataVector(fPulseData[fadc_data.chan].coarse_time,
                     fadc_data.coarse_pulse_time);
  PopulateDataVector(fPulseData[fadc_data.chan].fine_time,
                     fadc_data.fine_pulse_time);
...
void Fadc250Module::DecodePulseParameters(UInt_t pdat, uint32_t data_type_id) {
  ...
  if (data_type_id == 1) {
    ...
    PopulateDataVector(fPulseData[fadc_data.chan].pedestal,
                       fadc_data.pedestal_sum);
    PopulateDataVector(fPulseData[fadc_data.chan].pedestal_quality,
                       fadc_data.qual_factor);
    // Debug output
#ifdef WITH_DEBUG
    if (fDebugFile)
    ...
    assert(data_type_id == 0); // Ensure this is a data continuation word
    if (((pdat >> 30) & 0x1) ==
        1) { // Ensure that data is the integral of pulse window word
      fadc_data.sample_sum =
          (pdat >> 12) & 0x3FFFF; // 18-bit sum of raw samples that constitute
                                  // the pulse data set
      fadc_data.nsa_ext = (pdat >> 11) & 0x1; // NSA extended beyond PTW
      fadc_data.samp_overflow =
          (pdat >> 10) & 0x1; // One or more samples is overflow
      fadc_data.samp_underflow =
          (pdat >> 9) & 0x1; // One or more samples is underflow
      fadc_data.samp_over_thresh =
          (pdat >> 0) & 0x1FF; // Number of samples within NSA that the pulse is
                               // above threshold
      // Populate data vectors
      PopulateDataVector(fPulseData[fadc_data.chan].integral,
                         fadc_data.sample_sum);
      PopulateDataVector(fPulseData[fadc_data.chan].overflow,
                         fadc_data.samp_overflow);
      PopulateDataVector(fPulseData[fadc_data.chan].underflow,
                         fadc_data.samp_underflow);
      // Debug output
#ifdef WITH_DEBUG
      if (fDebugFile)
        *fDebugFile << "Fadc250Module::Decode:: FADC PULSE PARAMETERS"
                    ...
                    << fadc_data.samp_over_thresh << endl;
#endif
    }
    if (((pdat >> 30) & 0x1) ==
        0) { // Ensure that data is the time of pulse window word
      fadc_data.coarse_pulse_time =
          (pdat >> 21) & 0x1FF; // Coarse time (4 ns/count)
      fadc_data.fine_pulse_time =
          (pdat >> 15) & 0x3F;                    // Fine time (0.0625 ns/count)
      ...
      fadc_data.peak_beyond_nsa =
          (pdat >> 2) &
          0x1; // Pulse peak is beyond NSA or could be beyond window end
      fadc_data.peak_not_found =
          (pdat >> 1) & 0x1; // Pulse peak cannot be found
      fadc_data.peak_above_maxped =
          (pdat >> 0) &
          0x1; // 1 or more of first four samples is above either MaxPed or TET
      // Populate data vectors
      PopulateDataVector(fPulseData[fadc_data.chan].coarse_time,
                         fadc_data.coarse_pulse_time);
      PopulateDataVector(fPulseData[fadc_data.chan].fine_time,
...
void Fadc250Module::DecodeScalerHeader(UInt_t pdat) {
  ...
  fScalersToRead = fadc_data.scaler_words;     // Number of scaler words to read
...
void Fadc250Module::UnsupportedType(UInt_t pdat, uint32_t data_type_id) {
  ...
  static const vector<string> what_text{"UNDEFINED TYPE", "DATA NOT VALID",
                                        "FILLER WORD"};
  // Lookup table data_type -> message number
  static const map<uint32_t, uint32_t> what_map = {// undefined type
                                                   {5, 0},
                                                   {11, 0},
                                                   {13, 0},
                                                   // data not valid
  ...
  ostringstream str;
  str << "Fadc250Module::Decode:: " << what << " >> data = " << hex << pdat
...
Int_t Fadc250Module::Decode(const UInt_t *pdat) {
  ...
  // Intercept scaler data before type defining word
  if (fScalersToRead > 0) {
    if (fScalerData.empty()) {
      fScalerData.push_back(fadc_scaler_data{});
    }

    if (fScalersToRead == 1) {
      fScalerData.back().scaler_trigger_count = data;
...
#ifdef WITH_DEBUG
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::Decode:: FADC SCALER DATA"
                  << " >> data = " << data
                  << " >> remaining = " << fScalersToRead << endl;
#endif
    return block_trailer_found;
  }

  // Resume type tag identification
  uint32_t data_type_id =
      (data >> 31) & 0x1; // Data type identification, mask 1 bit
  if (data_type_id == 1)
  ...
  case 1: // Block trailer, indicates the end of a block of events
    ...
    break;
  case 2: // Event header, indicates start of an event, includes the trigger
          // number
    DecodeEventHeader(data);
    break;
  case 3: // Trigger time, time of trigger occurrence relative to the most
          // recent global reset
...
void Fadc250Module::LoadTHaSlotDataObj(THaSlotData *sldat) {
  ...
  for (uint32_t chan = 0; chan < NADCCHAN; chan++) {
    ...
    for (vsiz_t ievent = 0; ievent < fPulseData[chan].integral.size(); ievent++)
      sldat->loadData("adc", chan, fPulseData[chan].integral[ievent],
                      fPulseData[chan].integral[ievent]);
    // Pulse Time
    for (vsiz_t ievent = 0; ievent < fPulseData[chan].time.size(); ievent++)
      sldat->loadData("adc", chan, fPulseData[chan].time[ievent],
                      fPulseData[chan].time[ievent]);
    // Pulse Peak
    for (vsiz_t ievent = 0; ievent < fPulseData[chan].peak.size(); ievent++)
      sldat->loadData("adc", chan, fPulseData[chan].peak[ievent],
                      fPulseData[chan].peak[ievent]);
    // Pulse Pedestal
    for (vsiz_t ievent = 0; ievent < fPulseData[chan].pedestal.size(); ievent++)
      sldat->loadData("adc", chan, fPulseData[chan].pedestal[ievent],
                      fPulseData[chan].pedestal[ievent]);
    // Pulse Samples
    for (vsiz_t ievent = 0; ievent < fPulseData[chan].samples.size(); ievent++)
      sldat->loadData("adc", chan, fPulseData[chan].samples[ievent],
                      fPulseData[chan].samples[ievent]);
  } // Channel loop
}
