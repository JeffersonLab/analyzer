...
namespace Decoder {
  ...
  UInt_t Fadc250Module::GetData( Decoder::EModuleType emode, UInt_t chan, UInt_t ievent) const {
    switch( emode ) {
    case kSampleADC:
  ...
  UInt_t Fadc250Module::DecodeOneWord( UInt_t pdat)
  {
    ...
    if (fDebugFile)
      *fDebugFile << "Fadc250Module::Decode:: FADC DATA TYPES >> data = " << hex
                  << pdat << dec << " >> data word id = " << data_type_id
	...
	*fDebugFile << "Fadc250Module::Decode:: fSlot & FADC slot do not match AND data type != 0" << endl;
	...
	if (data_type_id) {
	  ...
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: FADC BLOCK HEADER >> data = " << hex
                        << pdat << dec << " >> slot = " << fadc_data.slot_blk_hdr
                        << " >> module id = " << fadc_data.mod_id << " >> event block number = " << fadc_data.iblock_num
                        << " >> num events in block = " << fadc_data.nblock_events << endl;
#endif
	}
	else {
	  fadc_data.PL = (pdat >> 18) & 0x7FF;  // Number of samples before trigger point for processing to begin, mask 11 bits
	  fadc_data.NSB = (pdat >> 9) & 0x1FF;  // Number of samples before threshold crossing to include processing, mask 9 bits
	  fadc_data.NSA = (pdat >> 0) & 0x1FF;  // Number of samples after threshold crossing to include processing, mask 9 bits
	  // Debug output
#ifdef WITH_DEBUG
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: FADC BLOCK HEADER >> data = " << hex
                        << pdat << dec << " >> NSB trigger point (PL) = " << fadc_data.PL
	...
	if (fDebugFile)
	  *fDebugFile << "Fadc250Module::Decode:: FADC BLOCK TRAILER >> data = " << hex
                      << pdat << dec << " >> slot = " << fadc_data.slot_blk_trl
	...
	if (fDebugFile)
	  *fDebugFile << "Fadc250Module::Decode:: FADC EVENT HEADER >> data = " << hex
                      << pdat << dec << " >> event header trigger time = " << fadc_data.eh_trig_time
	...
	if (fDebugFile)
	  *fDebugFile << "Fadc250Module::Decode:: FADC TRIGGER TIME >> data = " << hex
                      << pdat << dec << " >> trigger time word 1 = " << fadc_data.trig_time_w1
	...
	if (data_type_id == 1) {
	  ...
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: FADC WINDOW RAW DATA >> data = " << hex
                        << pdat << dec << " >> channel = " << fadc_data.chan
...
#ifdef WITH_DEBUG
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: UNDEFINED TYPE >> data = " << hex << pdat
	...
	if (data_type_id == 1) {
	  ...
	  if ( ((pdat >> 30) & 0x1) == 1) {                     // Ensure that data is the integral of pulse window word
	    fadc_data.sample_sum = (pdat >> 12) & 0x3FFFF;     // 18-bit sum of raw samples that constitute the pulse data set
	    ...
	    fadc_data.samp_over_thresh = (pdat >> 0) & 0x1FF;  // Number of samples within NSA that the pulse is above threshold
	    // Populate data vectors
	  ...
	  if ( ((pdat >> 30) & 0x1) == 0) {                       // Ensure that data is the time of pulse window word
	    ...
	    fadc_data.peak_beyond_nsa = (pdat >> 2) & 0x1;       // Pulse peak is beyond NSA or could be beyond window end
	    fadc_data.peak_not_found = (pdat >> 1) & 0x1;        // Pulse peak cannot be found
	    fadc_data.peak_above_maxped = (pdat >> 0) & 0x1;     // 1 or more of first four samples is above either MaxPed or TET
	    // Populate data vectors
	    PopulateDataVector(fPulseData[fadc_data.chan].coarse_time, fadc_data.coarse_pulse_time);
	    PopulateDataVector(fPulseData[fadc_data.chan].fine_time, fadc_data.fine_pulse_time);
	    PopulateDataVector(fPulseData[fadc_data.chan].time, fadc_data.time);
...
#ifdef WITH_DEBUG
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: DATA NOT VALID >> data = " << hex << pdat
                        << dec << " >> data type id = " << data_type_id << endl;
#endif
	}
        break;
...
#ifdef WITH_DEBUG
	  if (fDebugFile)
	    *fDebugFile << "Fadc250Module::Decode:: FILLER WORD >> data = " << hex << pdat
                        << dec << " >> data type id = " << data_type_id << endl;
#endif
	}
        break;
