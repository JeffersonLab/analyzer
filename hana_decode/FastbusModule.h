...
public:
   ...
   Bool_t IsSlot(UInt_t rdata) override { return (Slot(rdata)==fSlot); };
   UInt_t LoadSlot( THaSlotData *sldat, const UInt_t* evbuffer, const UInt_t *pstop) override;
   void   DoPrint() const override;

   UInt_t GetOpt( UInt_t rdata) const override { return Opt(rdata); };
   ...
   void SetSlot( UInt_t crate, UInt_t slot, UInt_t header = 0,
                 UInt_t mask = 0, Int_t modelnum = 0 ) override;
...
private:
   ...
   ClassDefOverride(FastbusModule,0)  // Fastbus module
